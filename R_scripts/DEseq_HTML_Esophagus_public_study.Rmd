...
readcounts_1<-read.table("public/featurecounts_new_flags_unstraded_exon_countreadpairs_multiplemapping_O_study1.txt", sep = '\t', row.names = 1, header = TRUE,check.names = FALSE)
readcounts_study_2<-read.table("public/featurecounts_new_flags_unstraded_exon_countreadpairs_multiplemapping_O_study_2.txt", sep = '\t', row.names = 1, header = TRUE,check.names = FALSE)
readcounts_study_3<-read.table("public/featurecounts_new_flags_unstraded_exon_countreadpairs_multiplemapping_O_study3.txt", sep = '\t', row.names = 1, header = TRUE,check.names = FALSE)
readcounts_study_4<-read.table("public/featurecounts_new_flags_unstraded_exon_countreadpairs_multiplemapping_O_study4.txt", sep = '\t', row.names = 1, header = TRUE,check.names = FALSE)
...
readcounts_1<-readcounts_1[,-c(1:5)]
readcounts_2<-readcounts_study_2[,-c(1:5)]
readcounts_3<-readcounts_study_3[,-c(1:5)]
readcounts_4<-readcounts_study_4[,-c(1:5)]
readcounts_com<-cbind(readcounts,readcounts_2,readcounts_3,readcounts_4)
...
indremoved = which(apply(readcounts_com, 1, function(x) all(x == 0)) )
readcounts_com= readcounts_com[ -indremoved, ]
...
#coldata<-filter(coldata,Group !="Study1_control")

coldata<-filter(coldata,Sample_ID !="HH01_25296_25297" & Sample_ID != "HH01_25531"& Sample_ID != "HH01_25643", Sample_ID != "HH01_23182"& Sample_ID != "HH01_27306_27307" & Sample_ID != "HH01_25421_25422" & Sample_ID!= "HH02_86-32412" )

coldata<-filter(coldata,Group !="Study1_control")


readcounts<-readcounts_com%>%dplyr::select(one_of(dput(as.character(coldata$Sample_ID))))
...
#sample: column, row: features
#batch <- c(rep(1, 4), rep(2, 4))
adjusted <- ComBat_seq(readcounts, batch=coldata$Study, group=NULL)
...
dd_Eso_EoE_FC_GERD <- DESeqDataSetFromMatrix(countData =adjusted,colData = coldata,design = ~ Group)

#normalized counts
dd_Eso_EoE_FC_GERD <- estimateSizeFactors(dd_Eso_EoE_FC_GERD)
#View(counts(dd_Eso))
normalized_counts_EoE_GERD <- counts(dd_Eso_EoE_FC_GERD, normalized=TRUE)
```
...
library(ggplot2)
library(ggrepel)
#rld_Eso_EoE_GERD_FC_group<-rlog(dd_Eso_EoE_FC_GERD, blind=TRUE)
rld_Eso_EoE_GERD_FC_group<-vst(dd_Eso_EoE_FC_GERD, blind=TRUE)
saveRDS(rld_Eso_EoE_GERD_FC_group,file="rld_public_vst.RDS")
#rld_Eso_EoE_GERD_FC_group<-readRDS("rld_Eso_EoE_GERD_FC_group_original.RDS")
pcaData <- plotPCA(rld_Eso_EoE_GERD_FC_group, intgroup=c("Group", "Timpoint_edited","CED_Nr","Timepoint","Location"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
#png("Kolja/PCA_plot.png", units="in", width=10, height=10, res=700)
#pcaData<-filter(pcaData,Group != "Study1_control")
pcaData<-filter(pcaData,Group != "functional disorders")
pcaData<-filter(pcaData,Group != "GERD")


ggplot(pcaData, aes(PC1, PC2, color=Group, shape=Timepoint,group=Group)) +
  geom_point(size=5) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
  coord_fixed()+geom_text_repel(aes(label= name),max.overlaps = Inf) +theme_bw()+stat_ellipse()

ggplot(pcaData, aes(PC1, PC2, color=Group, group=Group)) +
  geom_point(size=5) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
  coord_fixed()+theme_bw()+stat_ellipse()
...
library(umap)
normalized_counts <- assay(rld_Eso_EoE_GERD_FC_group) %>%
...
ggplot(umap_plot_df, aes(X1, X2, color=Group, shape=Location,group=Group)) +
geom_point(size=5) +
xlab(paste0("X1")) +
ylab(paste0("X2 ")) + 
coord_fixed()+geom_text_repel(aes(label= name),max.overlaps = Inf) +theme_bw()+stat_ellipse()+facet_wrap(~CED_Nr)

umap_plot_df<-filter(umap_plot_df,Group != "Study1_control")
umap_plot_df<-filter(umap_plot_df,Group != "functional disorders")
umap_plot_df<-filter(umap_plot_df,Group != "GERD")

ggplot(umap_plot_df, aes(X1, X2, color=Group,group=Group)) +
geom_point(size=5) +
xlab(paste0("X1")) +
ylab(paste0("X2 ")) + 
coord_fixed()+theme_bw()+stat_ellipse()
...
library(pheatmap)
rld_mat <- assay(rld_Eso_EoE_GERD_FC_group) 
...
#ead(rld_cor) 
my_sample_col <- as.data.frame(coldata_EoE_GERD_FC[,c(7,10)])
my_sample_col$Sample_ID<-coldata_EoE_GERD_FC$Sample_ID
...
coldata_EoE_GERD_FC<-filter(coldata,Sample_ID !="HH01_25296_25297" & Sample_ID != "HH01_25531"& Sample_ID != "HH01_25643", Sample_ID != "HH01_23182"& Sample_ID != "HH01_27306_27307" & Sample_ID != "HH01_25421_25422" & Sample_ID!= "HH02_86-32412" )
readcounts_EoE_GERD_FC<-readcounts_1 %>% dplyr::select(one_of(dput(as.character(coldata_EoE_GERD_FC$Sample_ID))))
readcounts_EoE_GERD_FC<-as.matrix(readcounts_EoE_GERD_FC)
dd_Eso_EoE_FC_GERD <- DESeqDataSetFromMatrix(countData = readcounts_EoE_GERD_FC,
                              colData = coldata_EoE_GERD_FC,
                              design = ~  Group)
#normalized counts
dd_Eso_EoE_FC_GERD <- estimateSizeFactors(dd_Eso_EoE_FC_GERD)
#View(counts(dd_Eso))
...
```{r}
rld_Eso_EoE_GERD_FC_group<-rlog(dd_Eso_EoE_FC_GERD, blind=TRUE)
saveRDS(rld_Eso_EoE_GERD_FC_group,file="rld_Eso_EoE_GERD_FC_group_6_removed_samples.RDS")
#rld_Eso_EoE_GERD_FC_group<-readRDS("rld_Eso_EoE_GERD_FC_group.RDS")
pcaData <- plotPCA(rld_Eso_EoE_GERD_FC_group, intgroup=c("Group", "Timpoint_edited","CED_Nr","Timepoint"), returnData=TRUE)
...
```{r, message=FALSE, echo=TRUE, warning=FALSE}
dds_EoE_GERDn <- DESeq(dd_Eso_EoE_FC_GERD,test = "LRT", reduced = ~ 1)
resultsNames(dds_EoE_GERDn)
saveRDS(dds_EoE_GERDn,file="Meta_data/Deseq_object_public.rds")
#dds_EoE_GERDn<-readRDS("Meta_data/Deseq_object_EoE_GERD.rds")

contrast <- c("Group", "EoE cohort", "Study4_control")
...
saveRDS(res_EoE_GERD_shrunken,file="Meta_data/result_shrunken_public.rds")
#res_EoE_GERD_shrunken<-readRDS("Meta_data/result_shrunken_Deseq_object_EoE_GERD.rds")



contrast_1 <- c("Group", "EoE cohort", "Study3_control")
res<-results(dds_EoE_GERDn,independentFiltering = FALSE)
res_EoE_GERD_unshrunken_EoE_FD  <- results(dds_EoE_GERDn, contrast = contrast_1, alpha = 0.05)
res_EoE_GERD_shrunken_EoE_FD <- lfcShrink(dds_EoE_GERDn, contrast=contrast_1,type="ashr")
#saveRDS(res_EoE_GERD_shrunken_EoE_FD,file="Meta_data/result_shrunken_Deseq_object_EoE_FD_96.rds")
#res_EoE_GERD_shrunken<-readRDS("Meta_data/result_shrunken_Deseq_object_EoE_GERD.rds")
#plotMA(res_EoE_GERD_unshrunken, ylim=c(-2,2))
plotMA(res_EoE_GERD_shrunken_EoE_FD, ylim=c(-2,2))
...
res_EoE_GERD_shrunken_EoE_FD$hgnc_symbol <- convertIDs(row.names(res_EoE_GERD_shrunken_EoE_FD), "ENSEMBL", "SYMBOL", org.Hs.eg.db)

res_EoE_GERD_shrunken_EoE_FD$entrezgen <- convertIDs(row.names(res_EoE_GERD_shrunken_EoE_FD), "ENSEMBL", "ENTREZID", org.Hs.eg.db)
resOrdered_EoE_FD <- res_EoE_GERD_shrunken[order(res_EoE_GERD_shrunken_EoE_FD$pvalue),]
...
res_EoE_GERD_shrunken_EoE_FD_1 <- res_EoE_GERD_shrunken_EoE_FD %>%
  data.frame() %>%
  rownames_to_column(var="geneID") %>% 
  as_tibble()
...
#Keep only the significant genes using our pre-defined thresholds
sigOE_EoE_FD <- res_EoE_GERD_shrunken_EoE_FD_1  %>%
        filter(padj < padj.cutoff & abs(log2FoldChange) > lfc.cutoff)


write.table(sigOE_EoE_GERD,file="Meta_data/Significant_genes_EoE_study1.tsv",sep="\t")
write.table(sigOE_EoE_FD,file="Meta_data/Significant_genes_EoE_study2.tsv",sep="\t")
...
##EOE and FD
res_EoE_GERD_shrunken_EoE_FD_2<-res_EoE_GERD_shrunken_EoE_FD_1 %>% drop_na(hgnc_symbol | padj)
...
top20_sigOE_genes_EoE_FD<- res_EoE_GERD_shrunken_EoE_FD_2%>% 
        arrange(desc(log2FoldChange)) %>% 	#Arrange rows by padj values
        pull(geneID) %>% 		#Extract character vector of ordered genes
        head(n=30) 		#Extract the first 30 genes
...
###EOE and FD
top20_sigOE__EoE_FD_norm <- normalized_counts_EoE_GERD_df %>%
        filter(geneID %in% top20_sigOE_genes_EoE_FD )
top20_sigOE__EoE_FD_norm_1 <- as.data.frame(top20_sigOE__EoE_FD_norm)
rownames(top20_sigOE__EoE_FD_norm_1)<-top20_sigOE__EoE_FD_norm_1[,1]
#top20_sigOE__EoE_GERD_norm_1<-top20_sigOE__EoE_GERD_norm_1[,-1]
top20_sigOE_genes_EoE_FD_GERD_norm<-rbind(top20_sigOE__EoE_FD_norm_1,top20_sigOE__EoE_GERD_norm_1)
top20_sigOE_genes_EoE_FD_GERD_norm<-distinct(top20_sigOE_genes_EoE_FD_GERD_norm, geneID, .keep_all = TRUE)

melted_norm_counts_EoE_GERD <-data.frame(melt(top20_sigOE_genes_EoE_FD_GERD_norm))
colnames(melted_norm_counts_EoE_GERD) <- c("geneID", "Sample_ID", "normalized_counts")
df3 <- full_join(melted_norm_counts_EoE_GERD, coldata_EoE_GERD_FC[,c(1,10)])
df3<-left_join(df3, res_EoE_GERD_shrunken_2[,c(1,3,7)])
...
df3_IL <- full_join(melted_IL_genes_norm_counts_EoE_GERD, coldata_EoE_GERD_FC[,c(1,10)])
...
top20_sigOE_genes_EoE_GERD_down <- res_EoE_GERD_shrunken_2%>% 
        arrange(log2FoldChange)%>% 	#Arrange rows by padj values
        pull(geneID) %>% 		#Extract character vector of ordered genes
        head(n=30) 		#Extract the first 30 genes


top20_sigOE_genes_EoE_FD_down<- res_EoE_GERD_shrunken_EoE_FD_2%>% 
        arrange(log2FoldChange) %>% 	#Arrange rows by padj values
        pull(geneID) %>% 		#Extract character vector of ordered genes
        head(n=30) 		#Extract the first 30 genes
...
normalized_counts_EoE_GERD_df<-setDT(normalized_counts_EoE_GERD_df, keep.rownames = "geneID") 
## normalized counts for top 20 significant genes
top20_sigOE__EoE_GERD_down_norm <- normalized_counts_EoE_GERD_df %>%
        filter(geneID %in% top20_sigOE_genes_EoE_GERD_down )
top20_sigOE__EoE_GERD_down_norm_1 <- as.data.frame(top20_sigOE__EoE_GERD_down_norm)
rownames(top20_sigOE__EoE_GERD_down_norm_1)<-top20_sigOE__EoE_GERD_down_norm_1[,1]
#top20_sigOE__EoE_GERD_norm_1<-top20_sigOE__EoE_GERD_norm_1[,-1]


###EOE and FD
top20_sigOE__EoE_FD_down_norm <- normalized_counts_EoE_GERD_df %>%
        filter(geneID %in% top20_sigOE_genes_EoE_FD_down )
top20_sigOE__EoE_FD_down_norm_1 <- as.data.frame(top20_sigOE__EoE_FD_down_norm)
rownames(top20_sigOE__EoE_FD_down_norm_1)<-top20_sigOE__EoE_FD_down_norm_1[,1]
#top20_sigOE__EoE_GERD_norm_1<-top20_sigOE__EoE_GERD_norm_1[,-1]
top20_sigOE_genes_EoE_FD_GERD_down_norm<-rbind(top20_sigOE__EoE_FD_down_norm_1,top20_sigOE__EoE_GERD_down_norm_1)
top20_sigOE_genes_EoE_FD_GERD_down_norm<-distinct(top20_sigOE_genes_EoE_FD_GERD_down_norm, geneID, .keep_all = TRUE)

melted_norm_counts_EoE_GERD_down <-data.frame(melt(top20_sigOE_genes_EoE_FD_GERD_down_norm))
colnames(melted_norm_counts_EoE_GERD_down) <- c("geneID", "Sample_ID", "normalized_counts")
df3_down <- full_join(melted_norm_counts_EoE_GERD_down, coldata_EoE_GERD_FC[,c(1,10)])
df3_down<-left_join(df3_down, res_EoE_GERD_shrunken_2[,c(1,3,7)])
...
IL_genes_EOE_FD<-filter(res_EoE_GERD_shrunken_EoE_FD_1,hgnc_symbol=="IL4"|hgnc_symbol=="IL5"|hgnc_symbol=="IL13"|hgnc_symbol=="CCL26"| hgnc_symbol=="CXCL1" )
write.table(IL_genes_EOE_FD,file="Meta_data/IL_genes_EoE_FD.tsv",sep="\t",quote = FALSE)
IL_genes_EOE_FD<-IL_genes_EOE_FD$geneID
IL_genes_sigOE_EoE_FD_norm<- normalized_counts_EoE_GERD_df %>%
        filter(geneID %in% IL_genes_EOE_FD)
IL_genes_sigOE_EoE_FD_norm_1<- as.data.frame(IL_genes_sigOE_EoE_FD_norm)
rownames(IL_genes_sigOE_EoE_FD_norm_1)<-IL_genes_sigOE_EoE_FD_norm_1[,1]

IL_genes_sigOE_EoE_GERD_FD_norm<-rbind(IL_genes_sigOE_EoE_FD_norm_1,IL_genes_sigOE__EoE_GERD_norm_1)
IL_genes_sigOE_EoE_GERD_FD_norm<-distinct(IL_genes_sigOE_EoE_GERD_FD_norm, geneID, .keep_all = TRUE)

melted_IL_genes_norm_counts_EoE_GERD <-data.frame(melt(IL_genes_sigOE_EoE_GERD_FD_norm))
colnames(melted_IL_genes_norm_counts_EoE_GERD) <- c("geneID", "Sample_ID", "normalized_counts")
df3_IL <- full_join(melted_IL_genes_norm_counts_EoE_GERD, coldata_EoE_GERD_FC[,c(1,10)])
df3_IL<-left_join(df3_IL, res_EoE_GERD_shrunken_1[,c(1,3,7)])
ggplot(df3_IL, aes(x = as.factor(hgnc_symbol), y = normalized_counts)) +
...
#omit rows with NA
library()
SigOE_filter<-na.omit(sigOE_EoE_GERD)


f<-plotCounts(dds_EoE_GERDn, gene="ENSG00000089157", intgroup=c("Group","Timpoint_edited"),returnData=TRUE) 
